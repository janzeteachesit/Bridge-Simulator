...
function end(){
	if(vertex_id!="" && vertex_id!=undefined){
		...
		var item_xpos = parseFloat(item.style.left.replace("px","")) + parseFloat(item.style.width.replace("px",""))/2;
		var item_ypos = parseFloat(item.style.top.replace("px","")) + parseFloat(item.style.height.replace("px",""))/2;
		...
		if(norm<parseFloat(item.style.width.replace("px",""))*0.7){	
...
function drawlines(id){
	...
	for(var i=0; i<node1.edges.length; i++){
		...
		if(document.getElementById(name)==null && document.getElementById(name2)==null){
			document.getElementById("lines").innerHTML+=
				"<div id="+name+" style='position:absolute; text-align:center;'>"+
					...
					"<p id="+name+"_so style='text-align:center;position:absolute; top:-10px; left:0px; right:0px;'></p>"+
				"</div>";
		}
		...
		lil.style.top=(obj1y+obj2y)/2+32+"px";
		lil.style.left=(obj1x+obj2x)/2-radius/2+30+"px";
...
function glow(){
	...
	for(var j=0; j<joints.length; j++){
		if(joints[j].edges.length==0){imgurl="url('images/"+joints[j].type+".png')";}
		else{imgurl="url('images/"+joints[j].type+"g.png')";}
		document.getElementById(joints[j].id).style.backgroundImage = imgurl;
	}
	for(var j=0; j<anchors.length; j++){
		
		if(anchors[j].edges.length==0){imgurl="url('images/anchor.png')";}
		else{imgurl="url('images/anchorg.png')";}
		document.getElementById(anchors[j].id).style.backgroundImage = imgurl;
	}
}
...
function click(e){
	...
	if(!connected && id.indexOf('-')!=-1){
		...
		var imgurl;
		if(index!=-1){list.splice(index,1); imgurl = "url(images/"+obj.type+".png)"; count--;}
		else{list.push(obj); imgurl = "url(images/"+obj.type+"g.png)"; count++;}
...
function key_short(e){
	switch(e.keyCode){
		case 66:case 98:connect('beam'); break;//beam 
		case 76:case 108:connect('load'); break; //load
		case 82:case 114:connect('reaction'); break; //reaction
		case 83:case 115:solve_all(); break; //solve
		case 74:case 106:start('joint'); offsetX=32; offsetY=32; drag(); break;
		case 65:case 97:start('anchor'); offsetX=32; offsetY=32; drag(); break;
	}
}
function unnode(e){
...
function buckling_force(E,I,k,L){
	return Math.PI*Math.PI*E*I/(k*k*L*L);
}
